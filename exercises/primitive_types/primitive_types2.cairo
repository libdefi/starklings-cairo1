...
fn main() {
    ...
    let mut is_alphabetic = 'J'; // Finish this line like the example! What's your favorite short string?
    // Try a letter, try a number, try a special character, try a short string!
    let mut your_character = 'JOJI';
    if is_alphabetic(
        ref your_character
    ) {
        ('Alphabetical!').print();
...
fn your_character(ref char: felt252) -> bool {
    if char >= 'JOJI' {
        return true;
    } 
    false
